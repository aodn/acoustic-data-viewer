...
<g:each in="${spectrogramList}" var="item">
  <div class="panel" style="width:${item.width}px;" >
	<a href="#" onclick="handleSpectroClick(event,'${item.id}');return false;" >
	  ...
	  <img class="lazy" id="acoustic_Spectrogram-${item.id}" src="${resource(dir: 'images', file: 'loaderBelt.gif')}" data-original="${deployment.dataPath.replaceAll("/\$", "")}/${item.subdirectory}/${item.filename}"  />
	  </div>
	</a>
