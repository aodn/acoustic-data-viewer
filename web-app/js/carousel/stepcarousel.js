...
var stepcarousel = {
	...
	addnavbuttons:function($, config, currentpanel){
		...
		config.$leftnavbutton.bind('click', function(){ 
            ...
            $('#navButtonL').hide(); // hide to stop maniac pressing
			stepcarousel.stepBy(config.galleryid, -config.defaultbuttons.moveby)
		})
		config.$rightnavbutton.bind('click', function(){ 
            //assign nav button event handlers
            $('#navButtonR').hide(); // hide to stop maniac pressing
			stepcarousel.stepBy(config.galleryid, config.defaultbuttons.moveby)
		})
    ...
    moveTo:function(galleryid, pindex){ /*User entered pindex starts at 1 for intuitiveness. Internally pindex still starts at 0 */
        var config=stepcarousel.configholder[galleryid]
        if (typeof config=="undefined"){
            //alert("There's an error with your set up of Carousel Viewer \""+galleryid+ "\"!")
            return
        ...
        config.$belt.animate({left: -endpoint+'px'}, 100, function(){config.onslideaction(this)})
        config.currentpanel=pindex
        this.statusreport(galleryid)
    },
	...
	stepBy:function(galleryid, steps, isauto){
		...
		if (config.panelbehavior.wraparound==true && config.panelbehavior.wrapbehavior=="pushpull" && (pindex==0 && direction=='forward' || config.currentpanel==0 && direction=='back')){ //decide whether to apply "push pull" effect
			config.$belt.animate({left: -config.paneloffsets[config.currentpanel]-(direction=='forward'? 100 : -30)+'px'}, 'normal', function(){
				config.$belt.animate({left: -endpoint+'px'}, config.panelbehavior.stepbyspeed, function(){config.onslideaction(this)})
			})
		}
		else
			config.$belt.animate({left: -endpoint+'px'}, config.panelbehavior.stepbyspeed, function(){config.onslideaction(this)})
